...
public class TakingTurnsQueueTests
{
    ...
    // Defect(s) Found: None, this one passed fine.
    ...
    // Defect(s) Found: None, passed without issues.
    ...
    // Run 10 times.
    // Expected Result: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Sue, Tim, Tim
    // Defect(s) Found: Tim had turns=0 (infinite) but got kicked out after his first turn.
    //   GetNextPerson only re-enqueued people with Turns > 1, so infinite turn people were just dropped.
    ...
    // Defect(s) Found: Same issue — Tim had turns=-3 (also infinite) and got removed
    //   after one turn because the code didn't handle negative turns as infinite.
